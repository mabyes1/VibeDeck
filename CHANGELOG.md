...
## 0.1.61 - 2026-09-11 - WebRTC 相依套件安全更新

- SIPSorcery 從 10.0.9 更新至 10.0.16，修補 SCTP 與 TURN UDP 可被惡意封包中斷的兩項高風險弱點。
- 套件安全稽核涵蓋直接與間接相依套件；日後若偵測到 High 或 Critical 弱點，建置會直接失敗，不再只留下容易忽略的警告。
- Windows 自含式安裝包另外固定使用已修補的 System.Private.Uri 4.3.2，避免舊版 SharpDX 相依鏈在正式打包時重新帶入已知弱點。
